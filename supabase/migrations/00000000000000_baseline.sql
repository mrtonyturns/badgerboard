...
-- REPLAY-ONLY baseline: supabase/schema.sql (the pre-migrations base tables:
-- candidates, dossiers, offices, elections, voters, …) transformed to be
-- idempotent so `supabase db reset` can rebuild from the chain end to end.
--
-- ⚠ DO NOT run this against production. Prod already has these objects, and
-- some baseline POLICIES were deliberately dropped by later migrations
-- (20260704000004 backend hardening) — replaying is safe because those
-- migrations run afterward and drop them again, but applying THIS FILE ALONE
-- to prod would resurrect dropped policies.
--
-- Also converges two columns prod gained by hand:
--   offices.current_officeholder (written by admin-elections call_race sync)
...
DROP POLICY IF EXISTS "Authenticated users can read elections" ON elections;
...
DROP TRIGGER IF EXISTS prospecting_lists_updated_at ON prospecting_lists;
...
-- Columns prod carries that predate the chain:
ALTER TABLE offices ADD COLUMN IF NOT EXISTS current_officeholder TEXT;
