...
function decideNotification(rawSub = {}, rawContest = {}, results = [], now = new Date()) {
  ...
  const prev = sub.last_snapshot && typeof sub.last_snapshot === 'object' ? sub.last_snapshot : null
...
async function resolveEmail(sb, userId, cache) {
  if (cache.has(userId)) return cache.get(userId)
  let email = null
  try {
    const { data, error } = await sb.auth.admin.getUserById(userId)
    if (!error) email = (data && data.user && data.user.email) || null
  } catch (e) {
 ...
 * @returns {Promise<{sent:number,failed:number,skipped:number,subscriptions:number,contests:number,notes:string[]}>}
 */
async function notifyContestChanges(sb, contestIds, { trigger = 'unknown', now = new Date() } = {}) {
  const out = { sent: 0, failed: 0, skipped: 0, subscriptions: 0, contests: 0, notes: [] }
  try {
    ...
    for (const sub of subs) {
      try {
        ...
        const res = await sendEmail({ to, ...built })
        if (res && res.error) {
          out.failed++
          out.notes.push(`${contest.office}: ${decision.kind} email to a subscriber failed (${String(res.error).slice(0, 120)}).`)
          continue  // no bookkeeping write — the next pass retries
        }
        out.sent++

        if (decision.patch) {
          const { error: uErr } = await sb.from('election_subscriptions')
            .update(decision.patch).eq('id', sub.id)
          if (uErr) out.notes.push(`${contest.office}: notification bookkeeping write failed — ${uErr.message}`)
        }
      } catch (e) {
        out.failed++
