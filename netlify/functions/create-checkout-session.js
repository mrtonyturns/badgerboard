...
import { CANDIDATE_PLAN_CONFIG, ACTION_PLAN_CONFIG, PLAN_CONFIG, BRACKET_CONFIG, hasFeature, normalizePlan } from '../../src/lib/tiers.js'
...
export function getPriceEnvKey(plan, bracket, billing) {
...
/**
 * Single source of truth for plan → Stripe price ID.
 * Resolution order: env var > v2 catalog > founder-era fallback.
 * Reused by admin-billing.js so an admin plan change resolves prices exactly
 * the way a self-serve checkout does.
 * @returns {{ key: string, priceId: string|null }}
 */
export function resolvePriceId(plan, bracket, billing = 'monthly') {
  const key = getPriceEnvKey(String(plan || '').toLowerCase(), bracket, billing)
  return { key, priceId: process.env[key] || STRIPE_PRICES_V2[key] || STRIPE_PRICES[key] || null }
}

/** Stripe recurring price → our billing-period key. */
export function billingPeriodFromPrice(price) {
  const r = price?.recurring
  if (!r) return 'monthly'
  if (r.interval === 'year') return 'annual'
  if (r.interval === 'month') {
    if (r.interval_count === 3)  return 'quarterly'
    if (r.interval_count === 6)  return 'semiannual'
    if (r.interval_count === 12) return 'annual'
  }
  return 'monthly'
}
...
export const handler = async (event) => {
  ...
  // have them; this is the matching server-side gate, so a hand-crafted POST
  // can't buy credits the buyer's plan can't use.
  const entitlementFeature = async (feature) => {
    const { resolveEntitlement } = await import('./_entitlements.js')
    const { plan } = await resolveEntitlement(authedUser)
  ...
  if (product === 'credits') {
    ...
    const credits = await entitlementFeature('creditPacks')
    if (!credits.allowed) {
      return {
        statusCode: 403,
        body: JSON.stringify({
          error: 'Profile credit packs are available on paid plans. Upgrade to Monitor or higher to buy credits.',
          plan: credits.plan,
        }),
      }
    }
    const envKey  = `STRIPE_PRICE_CREDITS_${pack.toUpperCase()}`
    const priceId = process.env[envKey] || STRIPE_PRICES_V2[envKey] || STRIPE_PRICES[envKey]  // env > v2 catalog > founder-era fallback
    if (!priceId) {
      return {
        statusCode: 500,
  ...
  if (product === 'bulk_credits') {
    ...
    const bulk = await entitlementFeature('bulkCredits')
    if (!bulk.allowed) {
      return {
        statusCode: 403,
        body: JSON.stringify({
          error: 'Bulk profile credits are part of the Action Campaign plan. Upgrade to buy bulk credits.',
          plan: bulk.plan,
        }),
      }
    }
  ...
  if (!VALID_PLANS.includes(rawPlan)) {
    return {
      statusCode: 400,
      body: JSON.stringify({ error: `Invalid plan "${rawPlan}". Must be one of: ${VALID_PLANS.join(', ')}` }),
    }
  }

  // Legacy keys (monitor/campaign/agency) are accepted at the door for old
  // bookmarks and stale client bundles, but they must not travel any further:
  //   • getPriceEnvKey('monitor', 'b2_5', 'monthly') builds
  //     STRIPE_PRICE_MONITOR_B2_5_M — a price that does not exist in either
  //     catalog, because 'monitor' is really the candidate plan c_monitor and
  //     candidate plans have no bracket at all. Checkout 500s.
  //   • planType() classifies EVERY legacy key as 'action', so a legacy
  //     'monitor' checkout stamped plan_type:'action' into the Stripe
  //     subscription metadata that the webhook copies straight into
  //     app_metadata — misclassifying the account for good.
  // Collapse to the canonical key here, using the same mapping as
  // netlify/functions/_entitlements.js LEGACY_ALIASES (tiers.js mirrors it).
