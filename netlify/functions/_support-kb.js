...
3b. Refreshes are stable by design. Every run is anchored to the previous projection for that district: the numbers reproduce unless the research turns up something new, per-candidate movement is capped per refresh, and the #1 spot does not flip on a statistical tie. Real news — a dropout, a new poll — still moves it. If the models disagree badly, the confidence margin widens and says so. Do not read a 1-point shift as movement.
3c. Vote-share respects the calendar. Before a contested primary: each party's field is shown separately — candidates only compete inside their own primary — with the November general outlook below. Once nominees are set: a straight general head-to-head.
3d. Local Intel: add your own signal — canvass tallies, internal numbers, mailers, photos, PDFs, notes — and hit Regenerate. The AI identifies which candidates your intel is actually about and adjusts THOSE candidates, scaled to how hard the evidence is: a full internal poll or complete canvass moves the needle several points, an anecdote moves it barely at all. Candidates your intel says nothing about do not get re-ranked as a side effect. Intel is unverified and self-interested by nature, so it never overrides published polling. Private to your account; your intel never touches the shared district baseline other users see.
