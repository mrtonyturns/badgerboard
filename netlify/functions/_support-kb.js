// _support-kb.js — Badger Board support knowledge base (v1.24)
...
3a. Vote-share is an ensemble: Grok, Perplexity, and Gemini each project it independently from the same research, and the app takes the MEDIAN of the models that named each candidate — one model's bad read gets outvoted, and a candidate one model forgets does not get scored zero. A candidate only one model invents gets dropped.
3b. Refreshes are stable by design. Every run is anchored to the previous projection for that district: a new run is blended toward the standing numbers rather than replacing them, per-candidate movement is capped per refresh, and the #1 spot does not flip on a statistical tie. Hit refresh twice in an hour and you should see the same board. Real news — a dropout, a new poll, a sustained shift the models keep reporting — still lands, over one or two refreshes rather than in one jump. If the models disagree badly, the confidence margin widens and says so. Do not read a 1-point shift as movement.
3c. Candidates who are out stay out. A withdrawn candidate is dropped from the field and their support redistributed. If one model in the ensemble has not caught up and still lists them, that alone will not put them back on the board — the rest of the ensemble and the standing baseline have to agree they are running. Nobody is ever shown at 0%.
3d. Vote-share respects the calendar. Before a contested primary: each party's field is shown separately — candidates only compete inside their own primary — with the November general outlook below. Once nominees are set: a straight general head-to-head.
3e. Local Intel: add your own signal — canvass tallies, internal numbers, mailers, photos, PDFs, notes — and hit Regenerate. The AI identifies which candidates your intel is actually about and adjusts THOSE candidates, scaled to how hard the evidence is: a full internal poll or complete canvass moves the needle several points, an anecdote moves it barely at all. Candidates your intel says nothing about do not get re-ranked as a side effect. Intel is unverified and self-interested by nature, so it never overrides published polling. Private to your account; your intel never touches the shared district baseline other users see.
