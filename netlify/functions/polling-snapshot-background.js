...
const SNAPSHOT_SCHEMA_NOTE = `{
  ...
  "vote_share": [ { "candidate": "name", "party": "Republican|Democrat|Independent|Other", "pct": number }, ..., { "candidate": "Undecided", "party": "None", "pct": number } ],
...
- vote_share: how the district would vote if the election were held today, using the ACTUAL likely 2026 matchup from the research (fall back to a generic R vs D ballot if no declared challenger). Include an "Undecided" line. Percentages MUST sum to 100 ±1.
...
function validateSnapshot(s) {
  ...
  if (!Array.isArray(s.vote_share) || s.vote_share.length < 2) return 'vote_share too short'
  const sum = s.vote_share.reduce((t, v) => t + (typeof v.pct === 'number' ? v.pct : NaN), 0)
  if (!(sum >= 98 && sum <= 102)) return `vote_share sums to ${sum}`
  if (!s.vote_share.every(v => v.candidate && v.party)) return 'vote_share entries malformed'
